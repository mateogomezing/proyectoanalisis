...
public class DAOCiudadDeterminada implements IDAOCiudadDeterminada {
    ...
    @Override
    public ArrayList<DTOCiudadDeterminada> BuscarCiudadDeterminadaMayorDTO(String ciudad) {
    ArrayList<DTOCiudadDeterminada> listaCiudades = new ArrayList<>();
        try (Connection con = Conexion.getConnection()) {
            PreparedStatement pstmt = con.prepareStatement("select r.id,r.fechaReserva,h.id,h.categoria,h.tipo,h.ubicacion,hu.cedula,hu.nombreCompleto,hu.nivelestudio,hu.estrato,r.estado from reservahospedaje r join hospedaje h on r.idHabitacion=h.id join huesped hu on r.idHuesped=hu.id where h.ubicacion=? order by r.fechaReserva desc");
            pstmt.setString(1, ciudad);
